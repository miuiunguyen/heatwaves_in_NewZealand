...
save_path <- "//wel-storage1/cphrwel/Projects/EHI/Domains/Climate Change/Heatwave project 2020/Auckland project/All R works - final/All_threshold_CHC - 3ybaselinemort"
mort_path <- "//wel-storage1/cphrwel/Projects/EHI/Domains/Climate Change/Heatwave project 2020/Data/2025/Mortality update"
...
#-------------------------------------------------------------
### Cantebury AREA: TEMPERATURE DATA FROM CHC AERO STATIONS: AGENT #4843
#-------------------------------------------------------------

all_temp_CHC <- all_temp %>%
...
percentile_95_1981_2010 <- all_temp_CHC %>%
...
percentile_95_1971_2010 <- all_temp_CHC %>%
  filter(year >= 1971, year <= 2010) %>%
  summarise(percentile_95 = quantile(Tmean, probs = 0.95, na.rm = TRUE)) %>%
  pull(percentile_95)

per_Tmax_99_1981_2010 <- all_temp_CHC %>%
  filter(year >= 1981, year <= 2010) %>%
  summarise(percentile_99 = quantile(Tmax, probs = 0.99, na.rm = TRUE)) %>%
  pull(percentile_99)

per_Tmax_99_1971_2010 <- all_temp_CHC %>%
  filter(year >= 1971, year <= 2010) %>%
  summarise(percentile_99 = quantile(Tmax, probs = 0.99, na.rm = TRUE)) %>%
  pull(percentile_99)

# calculate 85th threshold of EHF
ehf_threshold <- all_temp_CHC %>%
  mutate(date = as.Date(paste(year, month, day, sep = "-"))) %>% 
...
ehf_summary_long <- ehf_summary_long %>%
  mutate(
    x_max = case_when(
      ...
      str_detect(metric, "Days") ~ 60,
      str_detect(metric, "Excess Temperature") ~ 200
    )
  )
...
ehf_top10 <- ehf_hw_episode %>%
  arrange(desc(`Heatwave Excess temperature (°C)`)) %>%
  slice_head(n = 10) %>% mutate(approach = "EHF - All heatwaves")

# Severe HEATWAVE
ehf_severe_top10 <- ehf_severe_hw_episode %>%
  rename(`Heatwave Excess temperature (°C)` = `Severe Heatwave Excess temperature (°C)`) %>% 
  arrange(desc(`Heatwave Excess temperature (°C)`)) %>%
  slice_head(n = 10) %>% mutate(approach = "EHF - Severe heatwaves")
...
ehf_top_all <- bind_rows(ehf_top10, ehf_severe_top10)
  
# Scatter plot
ehf_worst_hw <- ggplot(ehf_top_all, aes(x = Duration, y = `Tmax (average)`)) +
  ...
  scale_size_continuous(range = c(2, 8), breaks = c(10, 50, 100), limits = c(0, 120)) +
  scale_x_continuous(
    limits = c(0, 25),
  ...
  theme(
    ...
    axis.title.y = element_text(face = "bold", size = 12, angle = 0, hjust = 0, vjust = 1.1, margin = margin(r = -70)),
    panel.grid.minor = element_blank(),
...
full_path <- file.path(save_path, "fig8_ehf_worst_heatwaves.png")

ggsave(full_path, plot = ehf_worst_hw, width = 8, height = 6, dpi = 300) + theme(
  plot.background = element_rect(fill = "white", color = NA),
  panel.background = element_rect(fill = "white", color = NA))
...
met_thres_Tmax_31 <- 31
met_thres_Tmax_32 <- 32
met_thres_Tmean_23 <- 23

# Threshold Tmax = 31/Tmean = 23
met_summer_3123 <- summer_data %>%
  mutate(met_one_day_alert = case_when(
      Tmax >= met_thres_Tmax_31 + 4 ~ "Extremely hot",
      Tmax >= met_thres_Tmax_31 + 1 ~ "Very hot",
      Tmax >= met_thres_Tmax_31 ~ "Hot",
      TRUE ~ "")) %>% 
  # --- two-day alert condition ---
  mutate(
    cond_two_day = if_else(Tmean >= pmax(21, met_thres_Tmean_23) & Tmax >= 31, 1, 0),
  ...
  mutate(met_excess_heat = pmax(0, 
                                pmax(ifelse(!is.na(met_one_day_alert), Tmax - met_thres_Tmax_31, NA),
...
# Threshold Tmax = 32/Tmean = 23
met_summer_3223 <- summer_data %>%
  mutate(met_one_day_alert = case_when(
    Tmax >= met_thres_Tmax_32 + 4 ~ "Extremely hot",
    Tmax >= met_thres_Tmax_32 + 1 ~ "Very hot",
    Tmax >= met_thres_Tmax_32 ~ "Hot",
    TRUE ~ "")) %>% 
  # --- two-day alert condition ---
  mutate(
    cond_two_day = if_else(Tmean >= pmax(21, met_thres_Tmean_23) & Tmax >= 32, 1, 0),
    met_two_days_alert = if_else(
      (cond_two_day == 1 & (lag(cond_two_day) == 1 | lead(cond_two_day) == 1)),
      1, 0)
  ) %>% ungroup() %>% 
  ...
  mutate(met_heat_day = if_else(met_alert > 0, 1, 0)) %>% 
  mutate(met_excess_heat = pmax(0, 
                                pmax(ifelse(!is.na(met_one_day_alert), Tmax - met_thres_Tmax_32, NA),
                                     Tmean - pmax(21, met_thres_Tmean_23),
                                     na.rm = TRUE))) %>% 
  arrange(station_id, year, month, day) %>%  # make sure data is ordered by date
  group_by(station_id) %>%                  # process each station_id separately
...
# Add met_summer_3123 sheet
if ("met_summer_3123" %in% names(wb)) removeWorksheet(wb, "met_summer_3123")
addWorksheet(wb, "met_summer_3123")
writeData(wb, "met_summer_3123", met_summer_3123)

# Add met_summer_3223 sheet
if ("met_summer_3223" %in% names(wb)) removeWorksheet(wb, "met_summer_3223")
addWorksheet(wb, "met_summer_3223")
writeData(wb, "met_summer_3223", met_summer_3223)
...
met_summer_all <- bind_rows(
  met_summer_3123 %>% mutate(threshold = "Tmax = 31C/Tmean = 23C"),
  met_summer_3223 %>% mutate(threshold = "Tmax = 32C/Tmean = 23C")
)
...
#-------------------------------------------------
# threshold = "Tmax = 31C/Tmean = 23C"
met_heatwaves_3123 <- met_summary_season %>%
  filter(threshold == "Tmax = 31C/Tmean = 23C") %>% 
...
# threshold = "Tmax = 32C/Tmean = 23C"
met_heatwaves_3223 <- met_summary_season %>%
  filter(threshold == "Tmax = 32C/Tmean = 23C") %>% 
  mutate(across(c(num_norm_episodes, num_norm_hw_days, total_norm_excess_temp), as.numeric)) %>%
  pivot_longer(
    cols = c(num_norm_episodes, num_norm_hw_days, total_norm_excess_temp),
    names_to = "metric",
...
# save the plot
full_path_1 <- file.path(save_path, "fig13a_met_heatwaves_3123.png")
full_path_2 <- file.path(save_path, "fig13b_met_heatwaves_3223.png")

ggsave(full_path_1, plot = met_heatwaves_3123, width = 10, height = 6, dpi = 300) + theme(
  plot.background = element_rect(fill = "white", color = NA),
  panel.background = element_rect(fill = "white", color = NA))

ggsave(full_path_2, plot = met_heatwaves_3223, width = 10, height = 6, dpi = 300) + theme(
  plot.background = element_rect(fill = "white", color = NA),
  panel.background = element_rect(fill = "white", color = NA))
...
#-------------------------------------------------------------------
## FIGURE 15: TOTAL HEATWAVES DAYS , BY SUMMER MONTH, 2000/01 -2020/21
#-------------------------------------------------------------------
...
# MetService
met_hw_by_month_3123 <- met_all_heatwave %>% 
  filter(threshold == "Tmax = 31C/Tmean = 23C") %>% 
  mutate(month = factor(month,
                        levels = c("November", "December", "January", "February", "March"))) %>% 
...
met_hw_by_month_3223 <- met_all_heatwave %>% 
  filter(threshold == "Tmax = 32C/Tmean = 23C") %>% 
  mutate(month = factor(month,
                        levels = c("November", "December", "January", "February", "March"))) %>% 
  ggplot(aes(x = month, y = met_perc_days)) +
...
full_path_3 <- file.path(save_path, "Fig15c_met_3123_hw_days_by_month.png")
full_path_4 <- file.path(save_path, "Fig15c_met_3223_hw_days_by_month.png")
...
ggsave(full_path_3, plot = met_hw_by_month_3123, width = 6, height = 4, dpi = 300) + theme(
  plot.background = element_rect(fill = "white", color = NA),
  panel.background = element_rect(fill = "white", color = NA))

ggsave(full_path_4, plot = met_hw_by_month_3223, width = 6, height = 4, dpi = 300) + theme(
  plot.background = element_rect(fill = "white", color = NA),
  panel.background = element_rect(fill = "white", color = NA))
...
combined_hw <- bind_rows(ehf_hw_episode %>% mutate(threshold = "Tmax = 31C/Tmean = 23C"),
                         ehf_hw_episode %>% mutate(threshold = "Tmax = 32C/Tmean = 23C"),
                         
                         ehf_severe_hw_episode %>% mutate(threshold = "Tmax = 31C/Tmean = 23C"),
                         ehf_severe_hw_episode %>% mutate(threshold = "Tmax = 32C/Tmean = 23C"),
...
# Combine all datasets
top10_combined_hw <- bind_rows(ehf_top10 %>% mutate(threshold = "Tmax = 31C/Tmean = 23C"),
                               ehf_top10 %>% mutate(threshold = "Tmax = 32C/Tmean = 23C"),
                               
                               ehf_severe_top10 %>% mutate(threshold = "Tmax = 31C/Tmean = 23C"),
                               ehf_severe_top10 %>% mutate(threshold = "Tmax = 32C/Tmean = 23C"),
...
mort_summary <- mort_summer %>%
  group_by(month) %>%
  summarise(
    count_obs_mort = sum(!is.na(observed_mortality)),
    obs_mort_avg = mean(observed_mortality, na.rm = TRUE),
    obs_mort_sd = sd(observed_mortality, na.rm = TRUE),
...
mort_total <- mort_summer %>%
  summarise(
    total_obs_count = sum(!is.na(observed_mortality)),
    overall_min     = min(observed_mortality, na.rm = TRUE),
    overall_max     = max(observed_mortality, na.rm = TRUE)
  )
...
mort_aver_by_month <- mort_summary_long %>% 
  ...
  scale_y_continuous(breaks = seq(0, 20, 2), limits = c(0, 10))  # set grid at 0,2,4,6,8,10
...
mort_aver_by_ss <- mort_summary_ss %>% 
  ...
  scale_y_continuous(breaks = seq(0, 20, 2), limits = c(0, 12))  # set grid at 0,2,4,6,8,10
...
# Plot
hw_mort_2019_20_ehf <- hw_mort_2019_20 %>% filter (threshold == "EHF")
hw_mort_ehf <- hw_mort_2019_20_ehf %>% 
  ggplot(aes(x = date)) +
  ...
  # Observed mortality
  geom_line(aes(y = observed_mortality, color = "Observed mortality")) +
  # Baseline mortality
  geom_line(aes(y = baseline3y_mortality, color = "Baseline mortality")) +
...
hw_mort_met_3123 <- hw_mort_2019_20 %>% filter (threshold == "Tmax = 31C/Tmean = 23C") %>% 
  ggplot(aes(x = date)) +
  # Grey rectangles for heatwave periods
  # Heatwave shading
  geom_tile(
    data = hw_mort_2019_20 %>% filter (threshold == "Tmax = 31C/Tmean = 23C") %>% filter(!is.na(heatwave_flag_met)),
    aes(x = date, y=(max_y_rounded / 2), height = max_y_rounded, 
  ...
  # Tmax (secondary y-axis)
  geom_line(aes(y = Tmax, color = "Tmax")) +
  scale_y_continuous(name = "MetService (Threshold: Tmax = 31C/Tmean = 23C)",
...
hw_mort_met_3223 <- hw_mort_2019_20 %>% filter (threshold == "Tmax = 32C/Tmean = 23C") %>% 
  ggplot(aes(x = date)) +
  # Grey rectangles for heatwave periods
  # Heatwave shading
  geom_tile(
    data = hw_mort_2019_20 %>% filter (threshold == "Tmax = 32C/Tmean = 23C") %>% filter(!is.na(heatwave_flag_met)),
    aes(x = date, y=(max_y_rounded / 2), height = max_y_rounded, 
        fill = heatwave_flag_met),
    width = 1, 
    alpha = 0.6,
  ...
  # Tmax (secondary y-axis)
  geom_line(aes(y = Tmax, color = "Tmax")) +
  scale_y_continuous(name = "MetService (Threshold: Tmax = 32C/Tmean = 23C)",
                     sec.axis = sec_axis(~ ., name = ""))+  # secondary axis
  scale_color_manual(values = c(
    "Observed mortality" = "darkgreen",
    "Baseline mortality" = "olivedrab3",
...
hw_mort_all <- plot_grid(
  hw_mort_ehf, hw_mort_met_3123, hw_mort_met_3223,
...
hw_mort_met_3123_17 <- hw_mort_2017_18 %>% filter (threshold == "Tmax = 31C/Tmean = 23C") %>% 
  ggplot(aes(x = date)) +
  # Grey rectangles for heatwave periods
  # Heatwave shading
  geom_tile(
    data = hw_mort_2017_18 %>% filter (threshold == "Tmax = 31C/Tmean = 23C") %>% filter(!is.na(heatwave_flag_met)),
    aes(x = date, y=(max_y_rounded / 2), height = max_y_rounded, 
        fill = heatwave_flag_met),
    width = 1, 
    alpha = 0.6,
...
hw_mort_met_3223_17 <- hw_mort_2017_18 %>% filter (threshold == "Tmax = 32C/Tmean = 23C") %>% 
  ggplot(aes(x = date)) +
  # Grey rectangles for heatwave periods
  # Heatwave shading
  geom_tile(
    data = hw_mort_2017_18 %>% filter (threshold == "Tmax = 32C/Tmean = 23C") %>% filter(!is.na(heatwave_flag_met)),
    aes(x = date, y=(max_y_rounded / 2), height = max_y_rounded, 
        fill = heatwave_flag_met),
    width = 1, 
    alpha = 0.6,
...
hw_mort_all_17 <- plot_grid(
  hw_mort_ehf_17, hw_mort_met_3123_17, hw_mort_met_3223_17,
  ncol = 1,                  # vertical stack
  align = "v",               # align vertically
  rel_heights = c(1.2,1,1.2), # adjust relative heights if needed
  labels = NULL
...
hw_mort_met_3123_15 <- hw_mort_2015_16 %>% filter (threshold == "Tmax = 31C/Tmean = 23C") %>% 
  ggplot(aes(x = date)) +
  # Grey rectangles for heatwave periods
  # Heatwave shading
  geom_tile(
    data = hw_mort_2015_16 %>% filter (threshold == "Tmax = 31C/Tmean = 23C") %>% filter(!is.na(heatwave_flag_met)),
    aes(x = date, y=(max_y_rounded / 2), height = max_y_rounded, 
        fill = heatwave_flag_met),
    width = 1, 
    alpha = 0.6,
...
hw_mort_met_3223_15 <- hw_mort_2015_16 %>% filter (threshold == "Tmax = 32C/Tmean = 23C") %>% 
  ggplot(aes(x = date)) +
  # Grey rectangles for heatwave periods
  # Heatwave shading
  geom_tile(
    data = hw_mort_2015_16 %>% filter (threshold == "Tmax = 32C/Tmean = 23C") %>% filter(!is.na(heatwave_flag_met)),
    aes(x = date, y=(max_y_rounded / 2), height = max_y_rounded, 
        fill = heatwave_flag_met),
    width = 1, 
    alpha = 0.6,
...
hw_mort_all_15 <- plot_grid(
  hw_mort_ehf_15, hw_mort_met_3123_15, hw_mort_met_3223_15,
  ncol = 1,                  # vertical stack
  align = "v",               # align vertically
  rel_heights = c(1.2,1,1.2), # adjust relative heights if needed
  labels = NULL
...
# -------------------------------------
# FIGURE 19 EXTRA: HEATWAVES EP DURING 2018/19 SEASON
#--------------------------------------------
hw_mort_2018_19 <- final_hw_mort %>% filter(season == "2018/19")
min_date <- min(hw_mort_2018_19$date, na.rm = TRUE)
max_date <- max(hw_mort_2018_19$date, na.rm = TRUE)
hw_mort_2018_19$date <- as.Date(hw_mort_2018_19$date)
hw_mort_2018_19 <- hw_mort_2018_19 %>%
  mutate(heatwave_flag = ifelse(ehf_heatwave == 1, "Heatwave", NA)) %>%
  mutate(heatwave_flag = forcats::fct_drop(factor(heatwave_flag))) %>% 
  mutate(heatwave_severe_flag = ifelse(ehf_severe_heatwave == 1, "Severe heatwave", NA)) %>%
  mutate(heatwave_severe_flag = forcats::fct_drop(factor(heatwave_severe_flag))) %>% 
...
# calculate max_y separately
max_y <- max(hw_mort_2018_19$Tmax, na.rm = TRUE)
max_y_rounded <- ceiling(max_y / 5) * 5

# Plot
hw_mort_ehf_18 <- hw_mort_2018_19 %>% filter (threshold == "EHF") %>% 
  ggplot(aes(x = date)) +
  # --- severe heatwave shading (draw first so it appears under light grey if overlap) ---
  geom_tile(
    data = hw_mort_2018_19 %>% filter(!is.na(heatwave_severe_flag)),
    aes(x = date, y = (max_y_rounded / 2), height = max_y_rounded,
        fill = heatwave_severe_flag),
    width = 1,
    alpha = 0.7,
  ...
  # Heatwave shading
  geom_tile(
    data = hw_mort_2018_19 %>% filter(!is.na(heatwave_flag)),
    aes(x = date, y=(max_y_rounded / 2), height = max_y_rounded, 
        fill = heatwave_flag),
    width = 1, 
    alpha = 0.6,
  ...
  # X-axis
  scale_x_date(date_labels = "%d-%m-%Y",
               breaks = seq(as.Date("2018-11-01"), as.Date("2019-03-31"), by = "7 days"),
               limits = as.Date(c("2018-11-01", "2019-03-31")),
               expand = c(0,0)) +
  # Theme
  theme_minimal(base_size = 14) +
  theme(
...
hw_mort_met_3123_18 <- hw_mort_2018_19 %>% filter (threshold == "Tmax = 31C/Tmean = 23C") %>% 
  ggplot(aes(x = date)) +
  # Grey rectangles for heatwave periods
  # Heatwave shading
  geom_tile(
    data = hw_mort_2018_19 %>% filter (threshold == "Tmax = 31C/Tmean = 23C") %>% filter(!is.na(heatwave_flag_met)),
    aes(x = date, y=(max_y_rounded / 2), height = max_y_rounded, 
        fill = heatwave_flag_met),
    width = 1, 
    alpha = 0.6,
...
hw_mort_met_3223_18 <- hw_mort_2018_19 %>% filter (threshold == "Tmax = 32C/Tmean = 23C") %>% 
  ggplot(aes(x = date)) +
  # Grey rectangles for heatwave periods
  # Heatwave shading
  geom_tile(
    data = hw_mort_2018_19 %>% filter (threshold == "Tmax = 32C/Tmean = 23C") %>% filter(!is.na(heatwave_flag_met)),
    aes(x = date, y=(max_y_rounded / 2), height = max_y_rounded, 
        fill = heatwave_flag_met),
    width = 1, 
    alpha = 0.6,
  ...
  theme(
    ...
    panel.grid.minor.x = element_blank(),
    plot.margin = margin(t = 0, r = 10, b = 0, l = 10)) + 
  labs(x = "Date (2018/19)", color = "")

# combine plots: use hw_mort_ehf and met
hw_mort_all_18 <- plot_grid(
  hw_mort_ehf_18, hw_mort_met_3123_18, hw_mort_met_3223_18,
  ncol = 1,                  # vertical stack
  align = "v",               # align vertically
  rel_heights = c(1.2,1,1.2), # adjust relative heights if needed
  labels = NULL
...
# save the plot
# Create a filename
file_name <- "Fig19_extra_hw_mort_2018_19.png"
# Full path
full_path <- file.path(save_path, file_name)
ggsave(full_path, plot = hw_mort_all_18 , width = 8, height = 11, dpi = 300) + theme(
  plot.background = element_rect(fill = "white", color = NA),
  panel.background = element_rect(fill = "white", color = NA)
)
...
make_summary <- function(data, approach) {
  ...
  data %>%
    ...
    dplyr::summarise(
      ...
      excess_sum   = sum(excess3y_mortality,    na.rm = TRUE),
      baseline_sum = sum(baseline3y_mortality,  na.rm = TRUE),
      observed_sum = sum(observed_mortality,  na.rm = TRUE),
      .groups = "drop"
    ) %>%
...
custom_colors <- c(
  ...
  "Tmax = 31C/Tmean = 23C"= "#ffc20c",
  "Tmax = 32C/Tmean = 23C"= "#f7914d"
)
...
season_ep_summary <- function(data, approach) {
  ...
  data %>%
    ...
    dplyr::summarise(
      ...
      excess_sum   = round(sum(excess3y_mortality, na.rm = TRUE), 1),
      baseline_sum = round(sum(baseline3y_mortality,  na.rm = TRUE),1),
      observed_sum = round(sum(observed_mortality,  na.rm = TRUE),1),
      .groups = "drop"
    ) %>%
    # convert -Inf (empty/all-NA groups) to NA so they don't poison sums
    dplyr::mutate(duration = dplyr::if_else(is.finite(duration), duration, NA_real_)) %>%
...
#-----------------------------------------------------------------------------
# Figure 21: Number of excess deaths, 2015/16, 2017/18, 2018/19 and 2020/21
#-----------------------------------------------------------------------------
...
fig21_filtered <- all_season_total %>% filter(season %in% c("2015/16", "2017/18","2018/19", "2019/20"))
fig21_201516 <- all_season_total %>% filter(season %in% c("2015/16"))
fig21_201718 <- all_season_total %>% filter(season %in% c("2017/18"))
fig21_201819 <- all_season_total %>% filter(season %in% c("2018/19"))
