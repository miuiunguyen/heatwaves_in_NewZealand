...
hw_mort_ehf <- hw_mort_2019_20 %>% filter (threshold == "EHF") %>% 
  ...
  # Baseline mortality
  geom_line(aes(y = baseline3y.mortality, color = "Baseline mortality")) +
...
make_summary <- function(data, approach) {
  ...
  data %>%
    ...
    dplyr::summarise(
      ...
      excess_sum   = sum(excess3y.mortality,    na.rm = TRUE),
      baseline_sum = sum(baseline3y.mortality,  na.rm = TRUE),
...
season_ep_summary <- function(data, approach) {
  ...
  data %>%
    ...
    dplyr::summarise(
      ...
      excess_sum   = round(sum(excess3y.mortality, na.rm = TRUE), 1),
      baseline_sum = round(sum(baseline3y.mortality,  na.rm = TRUE),1),
